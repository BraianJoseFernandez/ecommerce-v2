<div x-data="{ open: false }">
    <header class="bg-purple-600 shadow-sm">
        <x-container class="px-4 py-4">
            <div class="flex justify-between items-center space-x-8">
                ...
                <div class="flex-1 hidden md:block">
                    <x-input placeholder="Buscar por producto, tienda o marca" class="w-full" />
                </div>
                            ...
                            @guest
                                ...
                                <p class="text-sm text-center mt-2">
                                    ¿No tienes una cuenta? <a href="{{ route('register') }}"
                                        class="text-purple-600 hover:text-purple-700">Registrate</a>
                                </p>
                            </div>
                            @else
    ...
    <div x-show="open" x-on:click="open = false" class="fixed top-0 left-0 inset-0 bg-black bg-opacity-25 z-10" style="display: none;">
        <div class="fixed top-0 left-0 z-20">
            <div class="flex">
                <div class="w-screen md:w-80 h-screen bg-white">
                    <div class="bg-purple-600 text-white font-semibold px-4 py-3">
                        ...
                        <ul>
                            @foreach ($families as $family)
                                <li wire:mouseover="$set('family_id', {{ $family->id }})">
                                    <a class="flex items-center justify-between  px-4 hover:px-6 py-3 text-gray-700 hover:bg-purple-600 hover:text-white"
                                        href="">{{ $family->name }}
                    ...
                    <div class="h-[calc(100vh-52px)] overflow-auto bg-white px-6 py-8">
                        ...
                        <ul class="grid grid-cols-1 xl:grid-cols-3 gap-8">
                            @foreach ($this->categories as $category)
                                <li>
                                    <a href="" class="text-lg font-semibold text-purple-700 ">
                                        {{ $category->name }}
                                    </a>
                                    ...
                                    <ul class="mt-4 space-y-2">
                                        @foreach($category->subcategories as $subcategory)
                                            <li>
                                                <a href="" class="text-sm text-gray-700 hover:text-purple-700">
                                                    {{ $subcategory->name }}
                                                </a>
                                            </li>
                                        @endforeach
